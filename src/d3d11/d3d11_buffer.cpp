...
namespace dxvk {
  ...
  : D3D11DeviceChild<ID3D11Buffer>(pDevice),
    ...
    m_buffer = m_parent->GetDXVKDevice()->createBuffer(info, GetMemoryFlags(), DxvkMemoryStats::Category::AppBuffer);
  ...
  Rc<DxvkBuffer> D3D11Buffer::CreateSoCounterBuffer() {
    ...
    return device->createBuffer(info, VK_MEMORY_PROPERTY_DEVICE_LOCAL_BIT, DxvkMemoryStats::Category::AppBuffer);
  }
